...
class handler(BaseHTTPRequestHandler):
    ...
    def send_json_response(self, data, status_code=200):
        ...
        try:
            ...
            print(f"✅ Response sent: {status_code}, data length: {len(json_data)}")
        except Exception as e:
            print(f"❌ Error encoding response: {str(e)}")
            # Send a fallback error if JSON encoding fails
            fallback = json.dumps({"error": "Internal server error during response encoding"})
    ...
    def generate_readme_with_gemini(self, analysis_context: dict, project_name: str = None, include_demo: bool = False, num_screenshots: int = 0, num_videos: int = 0):
        ...
        try:
            print(f"Generating README for project: {project_name or 'Unnamed'}")
            ...
            **README Structure:**
            ...
            fallback_readme = f"""
            # {project_name or "Project README"}
            ...
            try:
                ...
                # Return fallback README instead of error
                return fallback_readme, None
            
        except Exception as e:
            print(f"README generation error: {str(e)}")
            return None, f"README generation failed: {str(e)}"